...
| 12-02-T3 | 02 | 1 | D-07 | T-12-18 | The full Phase 12 CSS contract, including the reduced-motion block that `page.emulateMedia` cannot reach in this provider. Block count asserted as exactly **4** via the FULL rule-opening literal (`split('@media (prefers-reduced-motion: reduce)').length - 1`) — never the bare phrase `prefers-reduced-motion`, which also occurs in rationale comments (`src/app.css:837`) and reads 4 against only 3 real blocks at baseline | unit (node) | `npx vitest run src/phase12-access-screens.spec.js` | ✅ new | ⬜ pending |
...
| 12-08-T1 | 08 | 4 | ACC-02 / D-11 | T-12-06, T-12-07d | Latched one-shot `invalidateAll()` on the snapshot-sourced ejection promotes `page.data.gated` to `true`, so both store readers short-circuit and `createSubscriber` releases the shared `lobby` subscription. `ejectionPromoted = true` precedes the call (unlatched = infinite loop) and the call site is reachable only from the snapshot-sourced branch | source contract | `npx vitest run src/phase12-access-screens.spec.js` | ✅ Plan 09 T1 | ⬜ pending |
...
**Sampling continuity check (Nyquist):** every one of the 21 tasks across the nine plans carries an
...
|----------|-------------|------------|-------------------|
| Live ejection on flip-to-private **and release of the guest's lobby subscription** | ACC-02 / D-11 | Needs two real browser sessions against one room; the socket store cache is per-client and subscription teardown is only observable at runtime | 1. Open the room as host and, in a second browser, as a guest spectator with spectating ON. 2. Toggle `open_spectating` OFF in the Host Console. 3. Confirm the guest lands on the 403 Guest Gate and their roster row disappears from the host's view. 4. **Then confirm the subscription actually released, not merely that the gate rendered:** with the guest still parked on the gate, make further roster changes as the host (add/remove a player, kick another spectator) and confirm the guest's client receives NO further roster broadcasts — check the guest's DevTools Network → WS frames for zero new `room/lobby` messages after the ejection frame. This is what `12-08-T1(e)`'s latched `invalidateAll()` buys; if new frames keep arriving, the leak is open even though the gate looks right. 5. Confirm exactly ONE `invalidateAll()` fired (one extra `load` request in Network, not a repeating stream) — a repeating pattern means the `ejectionPromoted` latch was dropped |
