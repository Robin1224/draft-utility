...
5. A subscription gate in **`src/routes/+layout.svelte`** — the *root* layout (D-19). **Do not create `src/routes/draft/[id]/+layout.svelte`**; see the correction note in D-19.
...
- **D-19: The fence widens to two additive snapshot fields and five realtime touch points.** Two research findings force this:
  ...
  - **`src/routes/+layout.svelte` must gate its subscription on `page.data.gated`.** svelte-realtime caches stream stores by `path + ':' + args` with a refcount, and the layout subscribes to the *same* `lobby(code)` store as the page.

    > **Path correction (2026-09-04, pattern-mapper):** an earlier draft of this decision named `src/routes/draft/[id]/+layout.svelte`. **That file does not exist** — `src/routes/+layout.svelte` is the only layout in the project, and it holds the shared subscription at line 17 (`const snap = $derived.by(() => (code ? fromStore(lobby(code)).current : null))`). Creating a route-level layout to host the gate would add a **second** subscriber to the same refcounted store and re-open the exact leak D-19 exists to close. Gate the existing root layout; create nothing. Once the D-01 guard throws, that shared store holds `{error: FORBIDDEN}` permanently — it has no `refetch`, and `invalidateAll()` only re-runs HTTP loads. **Without this gate, D-04's `RETRY_AS_GUEST()` is dead on arrival.** The same change also stops an ejected guest from continuing to receive every roster broadcast (`access`/`filter` in this library is subscribe-time only).
