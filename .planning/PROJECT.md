...
- ✓ **Phase 11: Terminal Modals** (complete 2026-08-21) — Both host pre-launch surfaces reskinned as Cyber terminal modals over the dimmed lobby, built on a shared `CyModal.svelte` (native `<dialog>`, esc/scrim/✕ dismissal with veto-able close, focus return, `::backdrop` scrim, 640px full-screen takeover, reduced-motion safe): Draft Settings (`~/draft/config.sh` — timer stepper clamped 10–120s step 5, script editor with drag + [↑]/[↓] reorder preserving the `{team, action}` shape, draft-copy `SAVE_CONFIG()`/`CANCEL` with dirty footer-swap confirm) and Host Console (`~/draft/host_console` — move/kick, amber captain hint, `CANCEL_ROOM` in footer, START_DRAFT gated in both bar and modal from one derive). LobbyHostBar slimmed to a launcher; frozen `$live` wiring byte-identical. MOD-01/02 validated. Suite green (195 passed, +20). *4 visual spot-checks (reduced-motion feel, real-mouse drag, 640px takeover, backdrop dim) in `11-HUMAN-UAT.md` for human UAT — non-blocking. Code review: 0 critical, 6 warnings (edge-case UX) in `11-REVIEW.md`.*
...
*Last updated: 2026-08-21 — Phase 11 (Terminal Modals) complete*
